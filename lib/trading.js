...
async function fetchMarketData(symbols) {
  try {
    ...
    return symbols.map((symbol) => ({ symbol, price: 0, bid: 0, ask: 0, source: 'fallback' }));
  }
}
...
function buildFundManagerRecommendation(settings, market = [], portfolioMetrics = {}) {
  const benchmarkText = (settings.targetBenchmarks || []).join(', ').toLowerCase();
  const riskProfile = String(settings.riskProfile || 'balanced').toLowerCase();
  const wantsGrowth = benchmarkText.includes('s&p') || benchmarkText.includes('ihsg') || benchmarkText.includes('crypto');
  const wantsDefensive = benchmarkText.includes('gold') || benchmarkText.includes('usd') || benchmarkText.includes('infl');

  const recommendations = {};
  const base = riskProfile === 'aggressive' ? 35 : riskProfile === 'conservative' ? 20 : 28;
  const alt = riskProfile === 'aggressive' ? 25 : riskProfile === 'conservative' ? 15 : 22;
  const utility = riskProfile === 'aggressive' ? 20 : riskProfile === 'conservative' ? 10 : 15;
  const stable = riskProfile === 'aggressive' ? 20 : riskProfile === 'conservative' ? 40 : 25;

  if (wantsGrowth && riskProfile !== 'conservative') {
    recommendations.BTCUSDT = {
      action: 'BUY',
      reason: `Core growth engine to beat IHSG, S&P 500, and the crypto top-100 benchmark while staying ahead of inflation targets.`,
      conviction: 'High',
      weight: base,
      timeline: 'medium-term',
    };
    recommendations.ETHUSDT = {
      action: 'BUY',
      reason: `Diversified crypto exposure that supports higher upside versus gold and USD while keeping risk controlled.`,
      conviction: 'Medium',
      weight: alt,
      timeline: 'medium-term',
    };
    recommendations.BNBUSDT = {
      action: 'HOLD',
      reason: `Selective utility exposure; maintain a smaller position until the fund manager sees clearer breakout versus the defensive gold/USD mandate.`,
      conviction: 'Low',
      weight: utility,
      timeline: 'short-term',
    };
    ...
    recommendations.BTCUSDT = {
      action: 'HOLD',
      reason: `Defensive posture favors gold, USD, and inflation protection over aggressive crypto beta.`,
      conviction: 'Medium',
      weight: stable,
      timeline: 'short-term',
    };
    recommendations.ETHUSDT = {
      action: 'HOLD',
      reason: `Reduce volatility and preserve capital until benchmarks show a stronger trend relative to inflation.`,
      conviction: 'Medium',
      weight: Math.max(10, stable - 10),
      timeline: 'short-term',
    };
    recommendations.BNBUSDT = {
      action: 'HOLD',
      reason: `Keep exposure neutral while the fund manager prioritizes capital preservation and benchmark-relative resilience.`,
      conviction: 'Low',
      weight: Math.max(5, stable - 20),
      timeline: 'short-term',
    };
  }
...
async function analyzeAndSuggest(symbols, settings) {
  ...
  if (!suggestion && hasOpenAIKey) {
    try {
      ...
      raw = `OpenAI fallback failed: ${error.message}`;
      suggestion = null;
    }
  }

  if (!suggestion) {
    suggestion = buildFundManagerRecommendation(settings, market, portfolioMetrics);
...
async function executeTrade(symbol, side, quantity) {
  try {
    ...
    return {
      simulated: true,
      status: 'dry-run',
      message: `Live trade skipped: ${error.message}`,
    };
  }
}
  ...
  return {
    portfolio: {
      ...
      outperform: {
        ...
        vsSP500: (parseFloat(portfolioPerformance) - benchmarks['S&P500'].change).toFixed(2),
...
async function runStrategy() {
  ...
  const analysis = await analyzeAndSuggest(symbols, settings);
...
module.exports = {
  ...
  fetchMarketData,
  getAccountBalances,
  buildFundManagerRecommendation,
  analyzeAndSuggest,
  executeTrade,
  runStrategy,
  readJSON,
