...
// ============================================================================
// PORTFOLIO REBALANCING & ALLOCATION MANAGEMENT
// ============================================================================

function calculateCurrentAllocation(portfolio) {
  /**
   * Calculate current allocation of each asset
   * Returns: { BTCUSDT: 45.5, ETHUSDT: 25.3, USDT: 29.2 }
   */
  const allocation = {};
  const total = portfolio.totalUsd || 1;

  Object.entries(portfolio.holdings).forEach(([symbol, holding]) => {
    allocation[symbol] = Number(((holding.value / total) * 100).toFixed(2));
  });

  return allocation;
}

function calculateTargetAllocation(benchmarks, settings) {
  /**
   * Calculate target allocation based on benchmark characteristics
   * Strategy: Diversify to beat all benchmarks
   * - Growth Assets (crypto): 70%
   *   - Bitcoin & similar: 35%
   *   - Altcoins & diverse: 35%
   * - Stable Assets: 30%
   *   - USDT/stablecoins: 30%
  ...
  const growthPercent = settings.allocationPercent || 70;
  const stablePercent = settings.stablePercent || 30;

  // Core growth assets to beat crypto benchmarks
  targetAllocation['BTCUSDT'] = growthPercent * 0.5;  // 35% for BTC
  targetAllocation['ETHUSDT'] = growthPercent * 0.3;  // 21% for ETH (top alt)
  targetAllocation['BNBUSDT'] = growthPercent * 0.15; // 10.5% for BNB (ecosystem)
  targetAllocation['SOLUSDT'] = growthPercent * 0.05; // 3.5% for SOL (momentum)

  // Stable allocation
  targetAllocation['USDT'] = stablePercent;  // 30% stablecoin
  ...
  Object.keys(targetAllocation).forEach(symbol => {
    targetAllocation[symbol] = Number((targetAllocation[symbol] / total * 100).toFixed(2));
  });

  return targetAllocation;
}

function calculateRebalancingNeeds(currentAllocation, targetAllocation) {
  /**
   * Compare current vs target and determine what needs rebalancing
   * Threshold: 5% deviation triggers rebalance
   */
  const rebalancingNeeds = {};
  const threshold = 5; // 5% threshold

  // Check all symbols in either allocation
  ...
  allSymbols.forEach(symbol => {
    ...
    if (Math.abs(deviation) > threshold) {
      rebalancingNeeds[symbol] = {
        current,
        target,
        deviation,
        action: deviation > 0 ? 'SELL' : 'BUY', // Overweight = sell, underweight = buy
...
function buildRebalancingPrompt(portfolio, currentAllocation, targetAllocation, benchmarks, rebalancingNeeds, settings) {
  /**
   * Create prompt for AI to validate and optimize rebalancing decisions
   */
  const currentText = Object.entries(currentAllocation)
    .map(([sym, pct]) => `${sym}: ${pct}%`)
    .join('\n  ');

  const targetText = Object.entries(targetAllocation)
    .map(([sym, pct]) => `${sym}: ${pct}%`)
    .join('\n  ');

  const rebalancingText = Object.entries(rebalancingNeeds)
    .map(([sym, need]) => `${sym}: ${need.action} ${need.amount.toFixed(2)}% (currently ${need.current}%, target ${need.target}%)`)
    .join('\n  ');

  const benchmarkText = Object.entries(benchmarks)
    .map(([key, b]) => `${b.name}: ${b.change24h}% (24h)`)
    .join('\n  ');

  return `You are BAZ HOLDING GROUP - a professional fund manager executing a rebalancing cycle.

MISSION: Beat these benchmarks with a balanced, diversified portfolio:
  ${benchmarkText}
...
REBALANCING NEEDED:
  ${rebalancingText || 'Portfolio already balanced'}

STRATEGY:
- Primary (Growth): ${settings.allocationPercent}% crypto assets
- Stable (Defensive): ${settings.stablePercent}% USDT/stablecoins
- Risk Profile: ${settings.riskProfile}
- Goal: Outperform IHSG, S&P500, Gold, Bitcoin, Top100Crypto, and USD simultaneously

REBALANCING DECISION:
For each asset needing rebalancing, provide:
{
  "SYMBOL": {
    "action": "BUY" | "SELL" | "HOLD",
    "execution_priority": "High" | "Medium" | "Low",
    "allocation_target": target_percentage,
    "reasoning": "Why this balance serves the benchmark-beating strategy"
  }
}

Remember: Rebalancing is about maintaining strategic allocation, not speculation.
Focus on diversification to beat multiple benchmarks simultaneously.`;
}

async function generateRebalancingDecision(portfolio, currentAllocation, targetAllocation, benchmarks, rebalancingNeeds, settings) {
  /**
   * Get AI decision on rebalancing
   */
  const prompt = buildRebalancingPrompt(portfolio, currentAllocation, targetAllocation, benchmarks, rebalancingNeeds, settings);
  ...
  if (useOpenClaw) {
    try {
      ...
      console.error('OpenClaw rebalancing failed:', error.message);
    }
  }

  // Fallback to OpenAI
  if (!decision && useOpenAI) {
    try {
      const openai = getOpenAIClient();
      const response = await openai.messages.create({
        model: 'gpt-4-turbo',
      ...
      console.error('OpenAI rebalancing failed:', error.message);
    }
  }

  // Fallback: use internal rebalancing needs
  if (!decision) {
    decision = {};
    Object.entries(rebalancingNeeds).forEach(([symbol, need]) => {
      decision[symbol] = {
        action: need.action,
        execution_priority: Math.abs(need.amount) > 10 ? 'High' : 'Medium',
        allocation_target: need.target,
        reasoning: `Rebalance from ${need.current.toFixed(1)}% to ${need.target.toFixed(1)}%`,
      };
    });
    source = 'fund-manager-internal';
  }
...
async function executeRebalancing(portfolio, rebalancingDecision, prices) {
  /**
   * Execute rebalancing trades based on AI decision
   */
  const executedTrades = [];

  if (!rebalancingDecision || typeof rebalancingDecision !== 'object') {
    return executedTrades;
  }

  for (const [symbol, decision] of Object.entries(rebalancingDecision)) {
    if (!decision || typeof decision !== 'object') continue;

    const action = String(decision.action || '').toUpperCase();
    const holding = portfolio.holdings[symbol];
    const targetPercent = decision.allocation_target || 0;

    if (action === 'SELL' && holding && holding.quantity > 0) {
      // Sell overweight position
      const targetValue = (portfolio.totalUsd * targetPercent) / 100;
      const currentValue = holding.value;
      const toSellValue = Math.max(0, currentValue - targetValue);
      const toSellQuantity = (toSellValue / holding.price) * 0.95; // 95% to account for slippage

      if (toSellQuantity > 0.001) {
        const result = await executeSell(symbol, toSellQuantity);
      ...
      const toBuyValue = Math.max(10, targetValue - currentValue); // Minimum $10

      if (toBuyValue >= 10) {
        const result = await executeBuy(symbol, toBuyValue);
        executedTrades.push(result);
      }
    }
  }
...
async function runAutonomousFundManager() {
  ...
  try {
    ...
    // 6. REBALANCING PHASE
    console.log('\n⚖️  Rebalancing portfolio for benchmark-beating strategy...');
    const currentAllocation = calculateCurrentAllocation(portfolio);
    const targetAllocation = calculateTargetAllocation(benchmarks, settings);
    const rebalancingNeeds = calculateRebalancingNeeds(currentAllocation, targetAllocation);

    console.log('   Current allocation:');
    Object.entries(currentAllocation).forEach(([sym, pct]) => {
      console.log(`     ${sym}: ${pct}%`);
    });

    console.log('   Target allocation:');
    Object.entries(targetAllocation).forEach(([sym, pct]) => {
      console.log(`     ${sym}: ${pct}%`);
    });

    const rebalanceDecision = await generateRebalancingDecision(
      portfolio,
      currentAllocation,
      targetAllocation,
      benchmarks,
    ...
    console.log(`   Rebalancing source: ${rebalanceDecision.source}`);

    // 7. Execute rebalancing trades
    console.log('\n🔄 Executing rebalancing trades...');
    const rebalancingTrades = await executeRebalancing(portfolio, rebalanceDecision.decision, prices);
    const rebalanceSuccess = rebalancingTrades.filter(t => t.success).length;
    console.log(`   Rebalancing: ${rebalanceSuccess}/${rebalancingTrades.length} trades executed`);

    // 8. Generate AI recommendations for additional opportunities
    console.log('\n🧠 Generating opportunistic trading recommendations...');
    const rec = await generateRecommendations(portfolio, marketStats, benchmarks, settings);
    console.log(`   Source: ${rec.source}`);

    // 9. Execute opportunistic trades based on recommendations
    console.log('\n🚀 Executing opportunistic trades...');
    ...
    const allTrades = [...rebalancingTrades, ...opportunisticTrades];
    const successful = allTrades.filter(t => t.success).length;
    console.log(`   Opportunistic: ${opportunisticTrades.filter(t => t.success).length}/${opportunisticTrades.length} trades`);
